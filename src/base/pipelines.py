...
def images_to_frame(data_path: os.PathLike) -> pd.DataFrame:
    ...
    res = pd.DataFrame({
        "image_path": fnames,
        "label": [f.split("/")[-2] for f in fnames]
    })
    res["target"] = [1 if x == "Fresh" else 0 for x in res["label"]]
    return res


def decode_image(filename: os.PathLike, label: int, image_size: Tuple[int, int] = (32, 32), n_labels: Optional[int] = None) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
...
def augment_image(image: tf.Tensor, label : Optional[tf.Tensor] = None) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
